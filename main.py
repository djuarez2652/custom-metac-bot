...
class SpringTemplateBot2026(ForecastBot):
    ...
    async def run_research(self, question: MetaculusQuestion) -> str:
        """
        Research Phase: Uses Gemini 2.5 Flash via OpenRouter with web search capabilities
        to search for information related to the forecasting question.
        """
        async with self._concurrency_limiter:
            prompt = clean_indents(
                ...
                Search the web for the latest information on this topic.
            ...
            # Research phase: Use Gemini 2.5 Flash via OpenRouter for research
    ...
    async def _binary_prompt_to_forecast(
        ...
        # Aggregate phase: Use GPT-5-mini for reasoning
        reasoning = await self.get_llm("default", "llm").invoke(prompt)
        logger.info(f"Reasoning for URL {question.page_url}: {reasoning}")
        # Format phase: Use GPT-5-mini for parsing/formatting
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["tournament", "metaculus_cup", "test_questions", "test_single_question"],
        default="tournament",
        help="Specify the run mode (default: tournament)",
    )
    args = parser.parse_args()
    run_mode: Literal["tournament", "metaculus_cup", "test_questions", "test_single_question"] = args.mode
    assert run_mode in [
        "tournament",
        "metaculus_cup",
        "test_questions",
    ...
    template_bot = SpringTemplateBot2026(
        research_reports_per_question=1,  # Single research run per question
        predictions_per_research_report=3,  # 3 forecasters per research report
        ...
        llms={
            # Phase 1: Research - Gemini 2.5 Flash for web search/research
            ...
            # Phase 2: Aggregate - GPT-5-mini for reasoning on research
            "default": GeneralLlm(
            ...
            # Phase 3: Format - GPT-5-mini (separate instance) for parsing/structuring
            "parser": GeneralLlm(
                model="openrouter/openai/gpt-5-mini",
                temperature=0,
                timeout=40,
            ...
            # Helper: Summarizer for combining multiple research reports (rarely used)
    ...
    if run_mode == "tournament":
        ...
        # Example questions are a good way to test the bot's performance on multiple questions
        EXAMPLE_QUESTIONS = [
    ...
    elif run_mode == "test_single_question":
        # Test with a single question - useful for quick testing of the two-phase architecture
        SINGLE_TEST_QUESTION = "https://www.metaculus.com/questions/578/human-extinction-by-2100/"  # Human Extinction - Binary
        template_bot.skip_previously_forecasted_questions = False
        template_bot.publish_reports_to_metaculus = False  # Don't submit during single question testing
        question = client.get_question_by_url(SINGLE_TEST_QUESTION)
        print(f"\n{'='*80}")
        print(f"Testing single question: {question.question_text}")
        print(f"{'='*80}\n")
        forecast_reports = asyncio.run(
